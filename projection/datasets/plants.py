...
class PlantsDataset(chainer.dataset.DatasetMixin):
...
if __name__ == "__main__":
    ...
    count = 0
    n_image_list = []
    filenames = glob.glob(root_path + '/*/*.*')
    for filename in filenames:
        extension = filename.lower().split('.')[-1]
        if extension not in ['jpg', 'jpeg', 'png']:     # There are a couple of files with other extensions (jpeg, png)
            continue
    ...
    n_image_list = np.array(n_image_list, np.str_)
    ...
    if preprocess_val:
        ...
        for filename in filenames:
            ...
            if extension not in ['jpg', 'jpeg', 'png']:     # There are a couple of files with other extensions (jpeg, png)
                continue
            filename = filename.split('/')
            image_name = filename[-1].strip(extension)
